...
MLdata <- dplyr::select(MLdata,Date,level,Price,new_vaccinations,new_cases,new_tests) %>% na.omit()
MLdata$level <- as.factor(MLdata$level)

# normalizing
normalize <- function(x) {
  return ((x - min(x)) / (max(x) - min(x))) }
Norm_MLdata <- as.data.frame(lapply(MLdata[3:5], normalize)) 
Norm_MLdata <- cbind(Norm_MLdata, MLdata[c("level", "Date")])
...
algo_df <- algo_df %>%
  ...
  mutate(profit.loss = replace_na(Price/lag(Price) - 1, 0),
         ...
         position = if_else(Average.50.moving > Average.200.moving & pred == "High" , 1, -1) %>% lag() %>% replace_na(0),
         
         # PNL calc
         PNL = position * profit.loss * 0.5)

# creating xts object
Algo_xts <- xts(algo_df$profit.loss, order.by = algo_df$Date)

########### These are the results for trading the algo for the S&P500

# overall profit (%) which is 22 percent profit in portfolio
(prod(algo_df$profit.loss + 1) - 1) * 100
