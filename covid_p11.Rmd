...
```{r}
# Basic box plot
ggplot(covid, aes(x=new_cases_smoothed, y=hosp_patients, na.rm = T)) + 
  geom_boxplot()
...
```{r}
boxplot(covid$new_cases_smoothed)
```
...
covid1_aggregate <- aggregate(new_cases_smoothed~month+year, 
                  covid1,
                  FUN = sum)
column_my <- covid1 %>% mutate(month_year = as.Date(covid1$date,  format="%y-%m"))
column_my
...
covid2 <- covid1_aggregate %>% mutate(month_year = as.Date(covid1_aggregate$concatenated,  format="%m-%y"))
...
```{r}
class(covid1_aggregate$new_cases_smoothed)
\
```
