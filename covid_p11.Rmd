...
## Visual for variables and price
### Visual of vac,tests,cases
```{r}
#colnames(df_q2)
df_q2_date = df_q2 %>% drop_na(new_cases, new_tests, new_vaccinations) %>% group_by(Date) %>% summarize(avg_new_cases = mean(new_cases), avg_new_tests = mean(new_tests), avg_new_vac = mean(new_vaccinations))
...
### scatter plot for each country (log(new vac, tests, price, cases))
```{r, warning=FALSE}
...
## Australia polynomial regression
- Jasmie and Christin
```{r}
glimpse(df_aus)
...
df_aus$Lnew_casesP = log(df_aus$new_cases/log(df_aus$new_cases))
df_aus = drop_na(df_aus, LpriceP, Lnew_vaccinationsP, Lnew_testsP, Lnew_casesP)

#M2 = lm(Price ~ polym(new_tests, new_vaccinations, degree=2, raw=TRUE), data=df_aus)
#summary(M2)

ggplot(data = df_aus) + geom_point(aes(x = Lnew_testsP, y = LpriceP), color = "light blue") +
  geom_point(aes(x = Lnew_vaccinationsP, y = LpriceP), color = "light green") +
  geom_point(aes(x = Lnew_casesP, y = LpriceP), color = "red") +
  ggtitle("Prices of stocks against new tests and new_vaccinations") + ylab("Price") +theme_classic()
```
