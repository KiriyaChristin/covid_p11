...
df_q2_time_cor <- data.frame(c(1), c(2), c(3), stringsAsFactors = FALSE)
colnames(df_q2_time_cor) = c("start_time", "Price~new_vacc", "new_tests~new_cases")

i = 1
while (i <= nrow_df_q2) {
  subset = df_q2_date[i:(i+29),]
  rbind(df_q2_time_cor, subset[1,1], cor.test(subset$avg_price,
                                           subset$avg_new_vac)$estimate,
        cor.test(subset$avg_new_tests, subset$avg_new_cases)$estimate)
  i = i + 30
}
glimpse(q2_time_cor)
```
...
```{r}
cor_df1 <- data.frame(c(2), stringsAsFactors = FALSE)
colnames(cor_df1) = c("Price~new_vacc")

cor_df2 <- data.frame(c(3), stringsAsFactors = FALSE)
colnames(cor_df1) = c("new_tests~new_cases")
